...
//typedef map<char,char>::iterator mi
int main(int argc, char *argv[]){
	...
	if (comp){ //-c failure
		...
		bool check=false;
		while(j < s1.length()) {
			if (storage[(unsigned char)i]==s1[j]){
				check=true;
				break;
			}
				j++;
			...
			if (check==false)
			storage[(unsigned char)i]=s2[j-1];
		}
	}
	if (del) { //-d
		...
		for (size_t j=0; j < storage.size();j++) {
			if (storage[(unsigned char)j]!=s1[i]){
				storage2.push_back(storage[(unsigned char)j]);
			}
	}
	storage=storage2;
	storage2= {};
	...
	for (size_t i=0; i < s1.length(); i++) {
		for (size_t j=0; j < storage.size();j++) {
			if (storage[(unsigned char)j]==s1[i]){
				storage[(unsigned char)j]=s2[i];
			}
	}
}
}
	for (size_t j=0; j < storage.size();j++) {
		cout << storage[(unsigned char)j];
	}
	cout << endl;
	return 0;
}
