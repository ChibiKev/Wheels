...
map<char,char> createmap(const string& s1, const string& s2){
	map<char,char> I;
	for (size_t i = 0; i < s1.length(); i++) {
		I[s1[i]] = s2[i];
	}
/*
	for(map<char,char>::iterator i = I.begin(); i != I.end() ; i++) {
		cout << "This is my map = " << (*i).first << ": "<<(*i).second << endl;
	}
*/
	return I;
}
...
int main(int argc, char *argv[])
{
	...
	while (fread(&d,1,1,stdin)){
		if(del){
			if(J.find(d) == J.end()) cout << d;
		}
		else if(comp) {
			if(J.find(d) == J.end()) cout << s2[s2.length()-1];
			else cout << d;
		}
		else {
			if(J.find(d) != J.end()) cout << (*J.find(d)).second();
			else cout << d;
		}
	}

	for(map<char,char>::iterator i = J.begin(); i != J.end() ; i++) {
		cout << "This is my map = " << (*i).first << ": "<<(*i).second << endl;
	}
	cout << comp << endl;
}
